...
>>>>>>> 8ad062d (Initial commit_4)
  ...
  const [speedFactor, setSpeedFactor] = useState(defaultSpeed)
...
<<<<<<< HEAD
  ...
  const checkCollisions = useCallback(() => {
    ...
    const carTop = screenHeight.current - 247
    const carBot = carTop + TILE_HEIGHT
...
<<<<<<< HEAD
  ...
  function onPointerUp(e) {
    ...
    if (Math.abs(dx) > 30) {
      if (dx < 0) setCarLane(l => { const n = Math.max(0, l-1); carLaneRef.current = n; return n })
      else        setCarLane(l => { const n = Math.min(2, l+1); carLaneRef.current = n; return n })
    }
    touchStartRef.current = null
    setBoosting(false)
    boostRef.current = false
  }
  function checkBoost(clientY) {
...
>>>>>>> 8ad062d (Initial commit_4)
  ...
  const lineBase = Math.round(8 + speedFactor * 14)
...
>>>>>>> 8ad062d (Initial commit_4)
        ...
        <div className="rc-slider-wrap">
          <input
            ...
            onChange={e => setSpeedFactor(e.target.value / 100)}
            className="rc-slider"
            orient="vertical"
          />
          <span className="rc-slider-label">x{speedFactor.toFixed(2)}</span>
        </div>
...
<<<<<<< HEAD
        ...
        <div
          ref={swipeAreaRef}
          className={`rc-swipe-area ${boosting ? 'rc-boosting' : ''}`}
          onPointerDown={onPointerDown}
          onPointerMove={onPointerMove}
          onPointerUp={onPointerUp}
