...
<<<<<<< HEAD
  ...
  // Card transform
  let cardStyle = {}
  if (swipeDir === 'left')       cardStyle = { transform: 'translateX(-120vw) rotate(-20deg)', transition: 'transform 0.35s ease' }
  else if (swipeDir === 'right') cardStyle = { transform: 'translateX(120vw) rotate(20deg)',   transition: 'transform 0.35s ease' }
  else if (swipeDir === 'up')    cardStyle = { transform: 'translateY(-120vh)',                 transition: 'transform 0.35s ease' }
...
<<<<<<< HEAD
            ...
            {!revealed && <p className="fc-tap-hint">Tap to reveal</p>}
          </div>
          ...
          {/* Mnemonic peek — only visible when swiping up */}
          {savedMnemonic && masterOpacity > 0.05 && (
            <div className="fc-mnemonic-peek" style={{ opacity: masterOpacity }}>💡 {savedMnemonic}</div>
          )}
        </div>
      ...
      {/* Action buttons */}
      {revealed && !animating && !detailOpen && (
        <div className="fc-actions">
          <button className="fc-btn fc-btn-unknown" onClick={() => advance('unknown')}>
            ✗<span>Unknown</span>
          </button>
          <button className="fc-btn fc-btn-master" onClick={() => advance('master')}>
            ⭐<span>Master</span>
          </button>
          <button className="fc-btn fc-btn-known" onClick={() => advance('known')}>
            ✓<span>Known</span>
          </button>
        </div>
      )}
